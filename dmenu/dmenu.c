...
static void
keypress(XKeyEvent *ev, int options)
{
	char buf[64];
	int len;
...
insert:
	...
	case XK_Return:
	case XK_KP_Enter:
		puts((outstring = (sel && !(ev->state & ShiftMask)) ? sel->text : text));
		char name[1000]	= {0};
		switch(options) {
			case WRITE:
...
FILE *convert_arg_fd(const char *text) {
	int fds[2];
	if (pipe(fds) < 0) return NULL;
	write(fds[1], text, strlen(text)); return fdopen(*fds,"rw");
}

char *input_format(const char *str)  {
	char *buffer = malloc(sizeof(char) * (strlen(str) + 4));
...
//
	...
	char *input = input_format(argv[1]);
	FILE  *f = convert_arg_fd(input);
	//free(input);
	if (fast && !isatty(0)) {
		grabkeyboard();
		readstdin();
